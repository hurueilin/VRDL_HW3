...
class NucleusDataset(utils.Dataset):
    ...
    def load_nucleus(self, dataset_dir, subset):
        ...
        assert subset in ["train", "val", "stage1_train", "stage1_test", "stage2_test", "myTrain", "myTest"]
        # subset_dir = "stage1_train" if subset in ["train", "val"] else subset
        subset_dir = "myTrain" if subset in ["val"] else subset
        dataset_dir = os.path.join(dataset_dir, subset_dir)

        if subset == "val":
            image_ids = VAL_IMAGE_IDS
        elif subset == 'myTest':
            image_ids = ["TCGA-A7-A13E-01Z-00-DX1", "TCGA-50-5931-01Z-00-DX1", 
            "TCGA-G2-A2EK-01A-02-TSB", "TCGA-AY-A8YK-01A-01-TS1", 
...
# def pycocotools_rle_encode(bimask):
#     assert bimask.ndim == 2, "Mask must be of shape [Height, Width]"
#     h, w = bimask.shape
#     bimask = np.asfortranarray(bimask)
#     # return _mask.encode(bimask.reshape((h, w, 1), order='F'))[0]
#     return _mask.encode(bimask.reshape((h, w, 1), order='F').astype('uint8'))[0]
...
def my_mask_to_rle(image_id, mask, scores):
    ...
    for o in order:
        ...
        m = m.astype(np.uint8)
        pycoco_rle = mask_util.encode(np.asfortranarray(m))
        # print(f'current image id: {image_id}')
        # print(pycoco_rle)
        output.append(pycoco_rle)
    return output
...
def detect(model, dataset_dir, subset):
    ...
    for image_id in dataset.image_ids:
        ...
        print('Now processing:', source_id)
        rle = my_mask_to_rle(source_id, r["masks"], r["scores"])
        ...
        for box, seg, score in zip(r['rois'], rle, r["scores"]):
            x1 = int(box[0])
            y1 = int(box[1])
            x2 = int(box[2])
            y2 = int(box[3])
